...
var utils = {
    ...
    uploadUserImage: function (file, userID, userImageDir, callback) {
        ...
        var newFilename = utils.encrypt(baseFileName + userID) + path.extname(fileName);
    ...
    checkForValidImage: function (file, callback) {
        if (!validate.isEmpty(file.user_image)) {
            var imageMessage = "";
    ...
    encrypt: function (text) {
        var cipher = crypto.createCipher(config.CRYPTO_ALGORITHM, config.CRYPTO_PASSWORD)
